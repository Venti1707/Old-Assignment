...
async function preparedoraDataAndFiles() {
    try {
        ...
        let index = 1;
        //I have prepared 12 design data for the user dora.
        ...
        for (index = 1; index <= 12; index++) {
            ...
            try {
                ...
                try {
                    ...
                    console.log(`dora_design${index}.png is is not uploaded. Did not create 1 record in file table.`)
                    console.log(error);
                }
            } catch (error) {
...
} //End of function preparedoraDataAndFiles
...
async function prepareAdminAlexData() {
    try {
        let createUserResult = await createUser('alex', 'alex@admin.com', '$2b$10$K.0HwpsoPDGaB/atFBmmXOGTw4ceeg33.WrxJx/FeC9.gCyYvIbs6', 1);
        console.log('Inspecting createUserResult variable\n', createUserResult)
    } catch (error) {
        console.log('Create alex user record has error\n', error);
        return;
    } finally {
        console.log('Data seeding of alex record in user table has completed.');
        return;
    }
} //End of function prepareAdminAlexData 
...
async function prepareDesignerUserData() {
    try {
        ...
        createUserResult = await createUser('shirley', 'shirley@designer.com', '$2b$10$K.0HwpsoPDGaB/atFBmmXOGTw4ceeg33.WrxJx/FeC9.gCyYvIbs6', 2);
        ...
        //for simulating the use case, whereby the administrator alex@admin.com login to change
...
async function startSeedingData() {
    try {
        await preparedoraDataAndFiles(); //Create records in user and file table to describe dora who has 12 file submissions.
        await prepareAdminAlexData(); //Create admin role user.
        await prepareDesignerUserData(); //Create additional 13 user records.
    } catch (error) {
